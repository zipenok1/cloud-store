...
import { IUsers, IloginForm, IloginRes, TregistrForm, TregistrRes } from "./type/auth.type"
...
export const profile = async (token: string): Promise<IUsers> => {
    axios.defaults.headers.Authorization = `Bearer ${token}`
    return (await axios.get('api/users/profile')).data
}